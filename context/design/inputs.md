...
`src/bin/fetch/`, `inputs/`

Inputs live at `inputs/<year>/<NN>.txt`, zero-padded, downloaded by the `fetch`
...
populated, and since that directory is gitignored, a fresh clone must run `fetch`
...
block `fetch`, the binary you need to fix the problem. And it could only check
...
A later idea, to have the build script run `fetch` when files are missing, has
the same flaw wearing a conditional. Running `fetch` means downloading, so
...
existing input file counts as a cached download, so `fetch` only fills gaps.
